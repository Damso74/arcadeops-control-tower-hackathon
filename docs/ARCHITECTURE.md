...
The diagram below traces a `POST /api/runner-proxy` request from the
jury's browser all the way to Gemini and back, including every retry,
deadline and gate. Times in parentheses are wall-clock numbers from the
`2026-05-13` smoke run (`run_id b06cb0f8d64143f8ad52dc780528e74a`).
...
sequenceDiagram
    ...
    User->>Front: Click "Run mission"
    Front->>Proxy: POST { mission, scenario }
    Note over Proxy: AbortSignal.timeout(85_000)
    Proxy->>Runner: POST /run-agent
    ...
    Planner-->>Orch: PlannerResult (5364 ms · ~1.7 k tokens)
    ...
    Front-->>User: render evidence timeline + verdict (BLOCKED · 17.6 s · $0.001001)
```
...
flowchart LR
    ...
    subgraph VultrFra["Vultr · Frankfurt (fra) · vc2-1c-2gb · 140.82.35.52"]
        OS[Ubuntu LTS]
...
| `RUNNER_URL` (Vercel)  | `http://140.82.35.52`                                  |
| Vultr instance id      | `12c32476-a13f-4f0c-a1d4-e4845643b37e`                 |
| Vultr region           | `fra` · Frankfurt                                      |
| Vultr plan             | `vc2-1c-2gb` · $5/month                                |
| Public IP              | `140.82.35.52`                                         |
...
Real numbers from `.smoke-response-vercel.json` (run id
`b06cb0f8d64143f8ad52dc780528e74a`):

- 7 `steps`
  - 1 `PLANNER` planning step (5 364 ms)
  - 5 `WORKER` `tool_call` steps: `crm.lookup`, `kb.search`,
    `email.draft`, `approval.request`, `audit.log`
  - 1 `WORKER` `conclusion` step (1 244 ms)
- `tokens_used`: 11 453
- `cost_usd`: 0.001001 (computed deterministically from
  `usage_metadata`)
- `is_mocked`: `false` (LIVE Gemini, no fallback)
- `model`: `gemini-2.5-flash`
- `runner`: `vultr` · `region`: `local` (placeholder; production sets
  `REGION=fra` via cloud-init in a follow-up)
...
flowchart LR
    ...
    subgraph VultrVM["Vultr VM (140.82.35.52)"]
        UFWGate{UFW · 22/80/443 only}
        CaddyHop[Caddy :80]
        RunnerSvc[FastAPI runner :8000<br/>appuser · non-root]
        VMEnv[(/opt/arcadeops/.env · 0600<br/>GEMINI_API_KEY)]
    end
    ...
    Browser --> FrontJS
    Browser --> ProxyFn
    ProxyFn --> UFWGate --> CaddyHop --> RunnerSvc
    ArcadeFn -. optional V4+V5 .-> Google
